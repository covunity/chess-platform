...
export async function listPublishedCourses(
  ...
  if (options.q) {
    // Escape SQL LIKE wildcards and PostgREST filter syntax chars
    const escaped = options.q.replace(/[%_,.()\[\]"\\]/g, c => `\\${c}`)
    query = query.or(`title.ilike.%${escaped}%,tags_text.ilike.%${escaped}%`)
  }
