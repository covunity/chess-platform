-- ── Migration 087: Multi-branch rewind siblings ──────────────────────────────
-- Replaces the DB-trigger-based single-sibling approach with an RPC.
-- The FE now parses the PGN tree, extracts all leaf-path branches, and calls
-- manage_rewind_siblings with N linear PGN strings. The RPC atomically
-- replaces all siblings and adjusts chapter positions.
...
--    p_branch_pgns — one linear PGN string per leaf path
--                    pass empty array ('{}') to delete all siblings
...
BEGIN
  ...
  -- Net position shift for the lessons that follow the source in this chapter.
  -- Done in a single UPDATE so PostgreSQL checks the unique constraint only
  -- after all rows are adjusted (no transient duplicates).
